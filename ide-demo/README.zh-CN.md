...
源文件 : README.md
源文件字节 : 16721
源文件 SHA256 : d47c371ade8f3ff5c2736512e41a1e8b04d5dd22a9cd5d0035f007bb9f5745f0
翻译日期 : 2026-09-22
...
**关于这一对文件的方向：中文是先写的，`README.md`（英文）是照它翻译出来的，所以
实测记录的原稿是这一份中文。** 上面开头那个块按仓库其余十一对文档的同一形状填写，
`源文件 : README.md` 指的就是那份英文翻译。
...
2. **语言这边**：编译器源码里有一个 `debug` 模式（同一个解释器，停下来，从目录里读
   ...
   *（2026-09-21 附注：这一段是当时的原始记录，保留不改。英文 `README.md` 里同一段
   已经按今天的编译器**更正**了——现在的 `debug` 是一个能用的协议调试器，语法是
   `vela debug <file.vel> <cmddir> [arg...]`，不带命令目录时它会在 stderr 上报
   `vela: panic: debug needs a command directory`，而不是"未知模式"。）*
