...
| **Python's syntax, strict semantics** | `vm.exe check` | conditions must be `bool`, `//` and `%` are floor, string `+` **joins two `str`s** (and `str + int` is refused, with no silent `toString`), a value-producing statement is refused — but a `str` bound to an `int` is still accepted by `check`, and an undeclared type name is caught by an emitter panic rather than by a diagnostic |
...
| string `+` | implicit concatenation | **joins two `str`s** (`"ab" + "cd"` is `"abcd"`), and `+=` appends to a `mut` binding; `str + int` is still refused; two literals side by side are refused too |
| `"a" "b"` | accidental concatenation | error — write `"a" + "b"` |
