...
|---|---|---|
| **faster than C++** | `powershell -ExecutionPolicy Bypass -File tools\bench.ps1 -Reps 7` | **not established, and currently false on matmul.**  Measured 2026-09-19 with the runtime's checks finally working: serial matmul 512² **loses 5x** (1.058 s against the C++ twin's 0.213 s), parallel matmul loses about 6.6x (0.128 s against 0.019 s), sieve loses 1.5x (0.0196 s against 0.0129 s), mandelbrot ties (0.0113 s against 0.0114 s).  The cause is measured, not guessed: the emitter writes **every array index expression twice**, so the checked index arithmetic is computed twice per element access, and that alone is the 6x (a hand-edited copy of the same emitted C with the index hoisted into one temporary runs matmul in 0.176 s — *beating* the C++ twin — while the arithmetic checks themselves turn out to cost nothing measurable).  The fix is an emitter fix, in progress; this row gets its numbers re-measured when it lands, and until then "faster than C++" is a goal with a diagnosis, not a result |
