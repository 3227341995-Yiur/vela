...
|---|---|---|
| **faster than C++** | `powershell -ExecutionPolicy Bypass -File tools\bench.ps1 -Reps 7` | **not established, and currently false on matmul.**  Measured 2026-09-19 with the runtime's checks finally working: serial matmul 512² **loses 5x** (1.058 s against the C++ twin's 0.213 s), parallel matmul loses about 6.6x (0.128 s against 0.019 s), sieve loses 1.5x (0.0196 s against 0.0129 s), mandelbrot ties (0.0113 s against 0.0114 s).  The causes are measured, not guessed, and there are two of them: the emitter writes **every array index expression twice**, so the checked index arithmetic runs twice per element access (worth 1.058 s → 0.550 s), and after that the checked index arithmetic itself is ~68% of what remains (0.550 s → 0.177 s if it goes away, against the twin's 0.213 s).  The first is an emitter fix, in progress; the second is `selfhost/ELISION_PLAN.md`.  Until both land, "faster than C++" is a goal with a diagnosis, not a result |
