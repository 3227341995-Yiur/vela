...
The resolution I intend, to be done in `check.vel` when that file is free (another
agent is in it), is **not** to invent an ABI:

* **Allow `str` as an `extern c` parameter**, and keep refusing it as a *return*.
  The narrow justification is that this does not let anyone call `printf`: a callee
  declared with `str` must itself be written against Vela's own `vela_str`, whose
  layout and passing convention are *measured*, not guessed (M1: `{ptr, i64}`, passed
  by reference under the MSVC ABI, returned through a hidden pointer).  A foreign
  library function still has to be declared with `i32`/`u8`/pointer-free scalars,
  exactly as today.
* A `str` **return** stays refused, because that would mean the callee allocating
  into Vela's string region — a different problem with a different answer.
* The two corpus cases that assert the old rule (`extern_str_parameter`,
  `extern_str_argument`) must be rewritten to say what is now true, and the refusal
  that remains must still have its own case.

Until that change lands, the shim cannot be called from Vela with a string
argument.  Measured: **six** of the shim's 48 declarations take a `str` —
`vshim_module_open`, `vshim_fn_declare`, `vshim_fn_define`, `vshim_string_global`,
`vshim_block_append`, `vshim_emit_object` — and the other **42 type-check**
(`vm.exe check` on them with a `main` appended prints `ok`, exit 0), so the mapping
table and the rest of the interface are already known-good.  The declarations are
generated by `tools\gen-shim-decls.ps1` from the header (with a `-Check` mode, so
the two sides cannot drift) and are **staged at `selfhost\llvm\llvm_shim.vel`, not in
`selfhost\parts\`**, because registering a part that the checker refuses would break
every build — and `tools\build.ps1` now refuses to build when a file in `parts\` is
missing from the linker's list, which is how that would have been discovered.
