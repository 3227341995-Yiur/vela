...
源文件 : ENUMS_PLAN.md
源文件字节 : 19903
源文件 SHA256 : b9c659ce2b33836a9d7942f77b6f49294f1ec89241bd6e27d2d65d74aeccdd9e
翻译日期 : 2026-09-24
...
状态：**已实现**（分支 `enums`）。本文作为设计记录保留，而不是改写成"建成之后长什么样"的描述。第 1–5 步全部落地：解析器与两个新关键字、resolver 与 checker、解释器、C 后端，以及 LLVM 后端的**具名拒绝**（那个文件自己的约定：宁可拒绝，不猜）。验收用例已经按 `tests/accept/` 头部写的规矩移出该目录、带 golden 进入 `tests/cases.txt`。

**计划在跑起来之前只是假设，而这一份里有六条主张没能存活。** 它们记在文末"计划错在哪里，实测"一节，连同语料不可能抓到的东西。读下文之前值得先知道其中两条：要编译这项特性，前端必须先把**节点池调大**（链接后的编译器需要约 66 600 个节点，而当时有 65 536 个）；以及这里的 C 表示是一个带标签的 `struct`，不是第 4 步草拟的那个 `typedef`。
...
下面每一条都是上文里的一条主张，而它没能经受住与代码树的接触。把它们留在这里而不是改掉，是因为下一个按行号引用 `parser.vel` 的计划应该知道这一份的行号是怎么用的。

1. **"语句形式完全不用碰词法器就能识别，关键字是以后另一件独立的事"（"工作顺序"第 1 项）——被否定。** 语句开头的 `match` 与两个合法程序有歧义：`match = 1` 是赋值，`match(x)` 是调用。解析器只能去猜。`enum` 与 `match` 现在是关键字（`keyword_id` 里的 24 和 25，`selfhost/vela.vel`），SPEC.md §1.3 已更新，并且有一条语料用例（`enum_keyword_reserved`）钉住 `mut match: int = 1` 不再被接受的那一天。
2. **C 表示不是第 4 步草拟的那个。** C 后端里根本没有 `typedef`（`grep typedef emit.vel` 一个都找不到），所以生成出来的类型写作 `struct vl_Shape`，与其它具名类型的拼法一致。union 的成员是 `v_<Variant>`，里面的字段继续带 `f_` 前缀，因为 emitter 的字段命名不会为了一个构造而被绕开。
3. **"switch 被生成成不带 `default`……所以给枚举加一个变体而不写它的分支，会由 `cl` 报出编译警告"（第 4 步）——是错的，而且实测过。** 标签是 `int64_t` 而不是 C 的 `enum`，C 无法知道少了哪个 case：手写的、有一个标签值没处理的 switch，`cl /W3 /std:c11` 编译它一片警告都不打印。不生 `default` 仍然值得做，也有 `dumps` 用例钉住它，但真正会响的那张网是 **checker 的穷尽规则**。声称有两张网；实际是一张，外加一个摘要。
4. **计划里没有一个字提到节点池，而这项特性装不进它。** 实测：带上枚举代码之后，`vm.exe parse selfhost/vm.vel` 以 `this file has too many syntax nodes` 拒绝，位置在 18 966 行里的第 18 663 行——约 66 600 个节点对 65 536 的池——而 token 池是 131 072 里用了 128 608（98% 满）。两个上限都抬高了（节点 65536 → 131072，token 131072 → 262144），并且节点*表*搬进了 arena 中间未分配的区域，因为它的尺寸是池大小乘以八（`MEM_NT` 3145728 → 1600000）。这件事必须是**先于**特性的独立提交：上一个修订的种子编译器无法解析需要超过 65536 个节点的源码，所以池必须由一个自身源码还足够小的构建来抬高。
5. **验收语料抓不到枚举表的 bug，而且真漏过去了一个。** `tests/accept/` 里每个程序都只声明**一个**枚举，所以一张所有变体列表都从记录 0 开始的表看起来是对的。有两个枚举时就不是：变体偏移是在"任何变体都还不存在"的那一趟里写下的，于是第二个枚举的穷尽性是拿第一个枚举的变体来判的——它拒绝了一个点出自己所有变体的 `match`，而它同样可能接受一个漏掉某个变体的 `match`。`tests/build/enum_two_enums.vel` 就是那条回归用例，而它是新的：计划表里没有它。
6. **两件小事。** `enum_nested_pattern.vel` 用了 `None`——一个 Vela 关键字——作为变体名，于是关键字冲突先于嵌套规则触发，那个用例从来没测到嵌套；语料里的副本把变体重命名了。还有 `enum_in_array` 不可能像表里写的那样是 `run`：C 后端不降低聚合元素的数组（对结构体它同样拒绝），所以那一行是 `ok`，而解释器的输出是手工对着文件头里的字节核对过的。

计划里有一条说对了，而且值得点名：**把标签放进负载块**——变体那些单元的第 0 格，而结构体读取器所谓的"字段数"就是它的步长——正是它让 `copy_value`、`assign_value`、`Array[E, N]` 以及栈帧逃逸规则原样可用。对它做的一处修正：标签是该变体的记录下标**加一**，好让 0 表示"从来没有被写入过变体"（一个零填充的数组元素），而不是指向第一个变体。
