...
源文件 : SPEC.md
源文件字节 : 29220
源文件 SHA256 : f1de822ede2924ea7a4b4a7c112888f3d36818a73ffe705a6d564f7cef10086c
翻译日期 : 2026-09-24
...
* **窄化绑定会被检查，而这个检查有两半。** 绑定正是那种「没有规则比较两个类型、位宽却
  ...
  * **值要到运行期才知道的窄化，就在运行期检查。** 有了 `mut big: int = 5000000000`，
    ...
    路径只给一个答案。
  * **未闭，2026-09-24 实测——这条规则还没有覆盖到经过元素或字段的写入。** 对
    `a[i] = big` 与 `p.x = big`，检查器接受，编译产物以上面那句停下，而解释器没有：
    `Array[i32]` 元素会打印 `705032704`，`i32` 字段会保留 `5000000000`。写在这里而不是
    留给别人去发现，因为这条规则的整个要点就是「答案不取决你问哪个前端」；编译器与语料都
    把它记为待偿的工作，等三条路径一致时这一条会被删掉。
