...
源文件 : SPEC.md
源文件字节 : 41560
源文件 SHA256 : 7b64154f555cc51186ff8bc75f3719c951bdbc35151ca50fd62c12e6cdca4477
翻译日期 : 2026-09-24
...
* **两个 `str` 上的 `+` 把它们连接起来**，`+=` 则向一个 `mut` 绑定追加：
  `"ab" + "cd"` 得到 `"abcd"`，而在 `mut s: str = "a"` 之后执行 `s += "b"`，
  `s == "ab"`。结果是一个位于永久内存里的新 `str`（§6 规则 3），所以它可以被返回、
  被比较、再被连接。它与 `concat` 内建函数做的是**同一个操作**（§8）——不管写哪一种
  拼法，都是同一次运行时调用——而 `str + int` 仍然被拒绝，
  `operator '+' mixes str and int`（§3.1）：这门语言里没有任何东西把数字转成文本，
  一个开始这么做的 `+` 就是一处静默的 `toString`。
* **没有可变字符串**：`str` 上的 `+=` 是造一个新字符串，没有任何东西能就地追加到已有
  的那个上面。也没有切片。
* 拼接要分配内存，所以在 `parallel for` 里被拒绝（§7）——那是该规则唯一在没有函数调用
  的情况下也要裁决的运算符。
...
* 两个 `str` 上的 `+` 是拼接（§1.5），而它与 `concat(a, b)` 是同一个操作（§8）。
  `+` 是唯一一个对 `str` 有含义的运算符；其他每一个仍然比较两个操作数的类型，
  所以 `str + int` 是混合类型错误（§3.1）。
...
* 函数体内被写入的标量必须是该次迭代私有的（在函数体内声明），否则该写入就是共享状态，
  循环被拒绝；
* 函数体内的调用必须是 `pure`。有两个被**点名**拒绝，因为 `parallel for` 恰恰是它们会
  撞车的地方：`print`，它的全部目的就是对共享流产生副作用；以及 `concat`——见下一条。
  §8 那张不纯表里的其余部分在这里**尚未**被拒绝，而这个缺口是被写出来的而不是被藏起来的：
  `read_text` 增长的是同一块字符串区域，而今天的 `check` 仍然接受它出现在并行体里；
* **函数体内禁止拼接，两种拼法都禁止。** 两个 `str` 上的 `a + b` 与 `concat(a, b)` 是
  同一个操作，所以它们得到同一个判定——
  `'parallel for' may not concatenate strings: every iteration would allocate out
  of the same string region`——而拒绝运算符却放行内建（或反过来），描述的就是这个后端，
  而不是这门语言。这两者触及的是永久字符串区域：它由一个 bump 指针来延伸，而 C 后端
  会把被接纳的 `parallel for` 交给 OpenMP，于是两次同时拼接的迭代都会读它、也都会写它，
  谁后到谁拿到的就是输家那份字节——一个静默错误字符串，而这正是这门语言拒绝交付的唯一
  结果。因此两个 `str` 上的 `+` **只对本规则而言**是不纯的：它对 `pure def` 仍然是纯的，
  因为两条规则问的是不同的问题，而这一条问的是每次迭代共享的状态。
...
| `unescape(s)` | 解码反斜杠转义 |
| `concat(a, b)` | 两个字符串拼接——与两个 `str` 上的 `a + b` 是同一个操作（§1.5），两个后端都把它降成同一次运行时调用，所以两种拼法不会漂移。结果是永久内存，像字面量一样。**不允许出现在 `parallel for` 里**（§7），两种拼法都不行：它在每次迭代共享的字符串区域里分配内存 |
| `interned(h)` | 字符串表查找 |
