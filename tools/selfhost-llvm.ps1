...
if ($failures.Count -gt 0) {
    ...
    Say '  the compiler own parser has and those do not.  Build a program out of'
    Say '  `selfhost/parts/vm_state.vel` + `parser.vel` + a driver that reads a file and prints'
    Say '  the first statement, then delete parser functions until the answer changes.'
    exit 1
}
Say 'RESULT: ok'
exit 0
